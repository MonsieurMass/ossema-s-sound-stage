...
const Merci = () => {
  ...
  useEffect(() => {
    document.title = `Merci — ${ossema.artist} | ${ossema.label}`;
    setEmail(sessionStorage.getItem("ossema:subscribed"));
  }, []);
  ...
  return (
    <main className="min-h-screen bg-vellum text-ink">
      ...
      <section className="px-6 md:px-10 py-24 md:py-32 text-center animate-fade-up">
        <p className="caption opacity-50 mb-6">Inscription confirmée</p>
        ...
        {email && (
          <p className="text-sm text-muted-foreground mb-4">
            Un email de confirmation arrive à{" "}
            <span className="text-ink font-medium">{email}</span>.
          </p>
        )}
        <p className="text-sm text-muted-foreground max-w-md mx-auto leading-relaxed">
          Voici votre accès en avant-première : les paroles complètes et le clip officiel.
        </p>
      </section>
      ...
      <section className="px-6 md:px-10 pb-24 md:pb-32">
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-end mb-8">
            <div>
              <p className="caption opacity-50 mb-3">Débloqué — 01</p>
      ...
      <section className="bg-ink text-vellum py-24 md:py-32 px-6 md:px-10">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="caption opacity-40 mt-16">
            © {new Date().getFullYear()} {ossema.label} — Écrit par {ossema.artist}
          </p>
        </div>
      </section>
